...
public class HealthWebService {
		...
		if(obj instanceof UpLoadDataVO[]) {
			...
			try {
				for(UpLoadDataVO upLoadDataVO : upLoadDataVOArray) {
					...
					String logId = healthInfoImportLogService.log(importHealthInfoBatchId,user,getIpAddress(),logContent);
	...
	@WebMethod
	public String download(@WebParam(name="token") String token, @WebParam(name="pageNum") Integer pageNum, @WebParam(name="pageSize") Integer pageSize) {
		User user = getUser(token);
		if(null == user) {
			logger.info("=======Web Service 您没有权限访问.");
			return "您没有权限访问.";
		...
		Page<OldPersonEntity> oldPersonCollection = oldPersonService.getLocalOldPersonWithArea(areaCollection, page.getInstance());
		...
		for(OldPersonEntity oldPersonEntity : oldPersonCollection.getContent()) {
			...
			FingerprintCollectEntity fingerprintCollect = fingerprintCollectService.getFingerprintCollectByOldPersonId(oldPersonEntity.getId());
		...
		logger.info("=======Web Service 老年人数据已加载完毕,共有加载" + oldPersonCollection.getSize() + "/" + oldPersonCollection.getTotalElements() + "位老年人.");
		return SerializerHelper.serialize(downloadCollection);
	}
	
	@WebMethod
	public String downloadByOldPersonId(@WebParam(name="token") String token, @WebParam(name="oldPersonId") String oldPersonId) {
		User user = getUser(token);
		if(null == user) {
			logger.info("=======Web Service 您没有权限访问.");
			return "您没有权限访问.";
	...
	private DownloadOldPersonFingerprintVO createDownloadOldPersonFingerprintVO(FingerprintCollectEntity fingerprintCollect) {
		...
		if(StringHelper.isNotEmpty(fingerprintCollect.getFingerVerifyState())) {
			downloadOldPersonFingerprintVO.setFingerState(FingerState.parse(Integer.parseInt(fingerprintCollect.getFingerVerifyState())));
		}
