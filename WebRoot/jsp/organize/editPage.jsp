...
$(function(){
	$('#areaCreateButton').click(function(){
		...
		$.ajax({
			type:"POST",
			url:'<c:url value="/organize/edit.do" />',
			dataType:'text',
	    ...
	    <form id="areaForm" method="post">
	    	...
	    	<table class="tableStyle">
				<tr>
					...
					<td width="30%"><input value="${organize.name}" data-options="required:true,validType:['blankSpace','minLength[2]','length[0,10]']" class="easyui-textbox" name="name" id="name" /></td>
				</tr>
				<tr>
					<td width="20%" class="right">区域类型</td>
					<td width="30%">
					<input class="easyui-combobox" name="type" id="type" value="${organize.type}"
						data-options="
						required:true,
						editable:false,
						valueField:'dicCode',
