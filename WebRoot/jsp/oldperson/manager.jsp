...
<head>
  ...
  <body class="easyui-layout">
	<table id="dg" title="老年人管理"  data-options="
				rownumbers:true,
				fit : true,
				singleSelect:false,
				autoRowHeight:false,
	...
	<div id="tb" style="padding:5px;height:auto">
		<div>
			<form name="verifyForm" id="verifyForm">
				...
				">
				<input prompt='老年人建模状态' class="easyui-combobox" name="modelingStatus" id="modelingStatus"
					data-options="
					editable:false,
					valueField:'dicCode',
					textField:'dicName',
				...
				">
				<!-- 
				<input prompt='老年人年龄段' class="easyui-combobox" name="ageRange" id="ageRange"
					data-options="
					editable:false,
					valueField:'dicCode',
					textField:'dicName',
				...
				">
				 -->
				<!-- 
				<input prompt='老年人所属地区' name="areaId" id="organizeTree"  class="easyui-combotree" data-options="
		...
		<div>
			...
			<a href="#" title="查看老年人详细" class="easyui-linkbutton" iconCls="icon-user" plain="true" onclick="view();">查看</a>
			<a href="#" title="标记老年人已经死亡" class="easyui-linkbutton" iconCls="icon-status_offline" plain="true" onclick="died();">死亡</a>
			...
			<view:security url="/oldperson/number/undo.do">
				<!-- <a href="#" title="查看老年人的操作日志" class="easyui-linkbutton" iconCls="icon-application_xp_terminal" plain="true" onclick="operatorLog()">日志</a> -->
			</view:security>
		</div>
	</div>
	...
	$(function(){
		$('#dg').datagrid({
			...
			onDblClickRow: function(rowIndex,rowData){
				...
				top.openDialog({onClose:function(){},width:'50%',height:'90%',title:'老年人档案信息',url:url});
			}
		});
	});
	...
	function createdToNone() {
		...
		if(row.length == 0 || row.length > 1){
			...
			$.messager.confirm("系统提示","您确定将老年人的已建模状态变更为未建模吗?",function(r){
				if(r) {
					$.ajax({
						type:"POST",
	...
	function noneToContCreated() {
		...
		if(row.length == 0 || row.length > 1){
			...
			$.messager.confirm("系统提示","您确定将老年人的未建模状态变更为无法建模吗?",function(r){
				if(r) {
					$.ajax({
						type:"POST",
						url:'<c:url value="/oldperson/updateModelingStatus.do" />',
	...
	function update(){
		...
		if(row.length == 0 || row.length > 1){
			...
			top.openDialog({onClose:function(){$('#dg').datagrid();},width:'40%',height:'80%',title:'老年人基本信息修改',url:url});
		}
	}
	function died() {
		var row  = $('#dg').datagrid("getSelections");
		if(row.length == 0 || row.length > 1){
			...
			$.messager.confirm("系统提示","您确定该老年人已经死亡吗?",function(r){
				if(r) {
					$.ajax({
						type:"POST",
	...
	function del() {
		...
		if(row.length == 0 || row.length > 1){
			...
			$.messager.confirm("系统提示","确认要删除该老年人吗?",function(r){
				if(r) {
					$.ajax({
						type:"POST",
